...
// ║  Replace src/NovaOS.jsx in your project with this.   ║
// ╚══════════════════════════════════════════════════════╝
...
const WALLPAPERS = {
  ...
  custom: { name:"Custom", preview:"conic-gradient(#888 0deg,#555 360deg)" },
};
...
const acFill = (ac) => `rgba(${rgb(ac)},0.16)`;
const acBorder = (ac) => `rgba(${rgb(ac)},0.55)`;
const isUrl = (s) => /^https?:\/\//i.test(s) || /^[\w-]+\.[\w]{2,}(\/|$)/.test(s.trim());
...
function Wallpaper({ id, customUrl }) {
  ...
  if (!id || id==="bliss" || !w || !w.grad) return <BlissBackground/>;
  return <div style={{position:"absolute",inset:0,background:w.grad}}/>;
}

// ─── GLOBAL CSS ───────────────────────────────────────────────────────────────
// Fonts: Space Grotesk (UI) + DM Sans (body) + JetBrains Mono (terminal)
// Both are noticeably narrower than Syne/Figtree used in v3.0.
const CSS = `
  ...
  *{box-sizing:border-box;}
  ...
  input::placeholder,textarea::placeholder{color:rgba(255,255,255,0.22);font-family:'DM Sans',sans-serif;}
  textarea{resize:vertical;}
  @keyframes boot-in {from{opacity:0;transform:translateX(-8px)}to{opacity:1;transform:none}}
  @keyframes win-in  {from{opacity:0;transform:scale(0.96) translateY(6px)}to{opacity:1;transform:none}}
  @keyframes menu-up {from{opacity:0;transform:translateY(8px)}to{opacity:1;transform:none}}
  @keyframes toast   {from{opacity:0;transform:translateX(14px)}to{opacity:1;transform:none}}
  @keyframes spin    {from{transform:rotate(0deg)}to{transform:rotate(360deg)}}
  .dsk-icon:hover{background:rgba(255,255,255,0.16)!important;}
  ...
  .paint-sw:hover{transform:scale(1.22);z-index:2;}
  .file-row:hover{background:rgba(255,255,255,0.07)!important;}
  .srcard:hover{background:rgba(255,255,255,0.06)!important;}
  .bm-pill:hover{background:rgba(255,255,255,0.12)!important;}
  .ac-preset:hover{transform:scale(1.15);}
  .wp-swatch:hover{border-color:rgba(255,255,255,0.5)!important;}
  .tog-btn:hover{opacity:0.85!important;}
`;
...
const sHd = { fontFamily:FFB, fontWeight:600, fontSize:11, letterSpacing:1.5, color:"rgba(255,255,255,0.3)", marginBottom:12, textTransform:"uppercase" };
...
export default function NovaOS() {
  ...
  useEffect(() => {
    if (user && settings.wallpaper==="custom") {
  ...
  const handleAuth = async () => {
    ...
    if(mode==="register") {
      ...
      setUser(u); setData(d||{notes:[],tasks:[],wallpaper:"bliss",bio:"",joined:Date.now(),settings:{}}); setScreen("desktop");
    }
    setBusy(false);
  };
  ...
  const baseFontSize = fontSize==="large" ? 15 : 13;
  ...
  if (screen==="boot") return (
    <div style={{width:"100%",height:"100vh",background:"#050508",display:"flex",flexDirection:"column",justifyContent:"center",padding:"10vh 12%"}}>
      ...
      <div style={{fontFamily:FF,fontWeight:500,fontSize:12,color:"rgba(255,255,255,0.22)",letterSpacing:5,marginBottom:48}}>OPERATING SYSTEM  ·  v3.1</div>
      {bootLines.map((l,i)=>(
        <div key={i} style={{fontFamily:FFM,fontSize:12,color:l.includes("ready")?"#4f9eff":"rgba(255,255,255,0.4)",marginBottom:5,animation:"boot-in 0.13s ease-out"}}>
    ...
    <div style={{width:"100%",height:"100vh",position:"relative",overflow:"hidden"}}>
      ...
      <div style={{position:"absolute",inset:0,display:"flex",alignItems:"center",justifyContent:"center"}}>
        <div style={{background:"rgba(8,10,22,0.86)",backdropFilter:"blur(24px)",border:"1px solid rgba(255,255,255,0.11)",borderRadius:16,padding:"44px 40px",width:376,boxShadow:"0 40px 100px rgba(0,0,0,0.6)",position:"relative",overflow:"hidden"}}>
          ...
          {err&&<div style={{color:"#ff7878",fontFamily:FF,fontSize:13,textAlign:"center",marginTop:12}}>⚠ {err}</div>}
  ...
  // ── DESKTOP ────────────────────────────────────────────────────────────────
  const wpId = settings.wallpaper || data?.wallpaper || "bliss";
  return (
    <div style={{width:"100%",height:"100vh",position:"relative",overflow:"hidden",cursor:drag?"grabbing":"default",fontSize:baseFontSize}}>
      <style>{CSS}</style>
      <Wallpaper id={wpId} customUrl={customWp}/>

      {toast&&<div style={{position:"fixed",top:14,right:14,zIndex:99999,padding:"10px 18px",background:"rgba(8,10,22,0.97)",border:`1px solid ${AC}`,borderRadius:9,fontFamily:FFB,fontWeight:600,fontSize:13,color:"#fff",animation:"toast 0.17s ease-out",boxShadow:"0 8px 36px rgba(0,0,0,0.6)"}}>
        {toast}
      </div>}
      ...
      <div style={{position:"absolute",top:14,left:10,display:"flex",flexDirection:"column",gap:3,zIndex:1}}>
        {APPS.map(app=>(
          <div key={app.id} className="dsk-icon" onDoubleClick={()=>openApp(app.id)} title={app.desc}
            ...
            <span style={{fontFamily:FF,fontWeight:600,fontSize:10,color:"#fff",textAlign:"center",lineHeight:1.2,textShadow:"0 1px 4px rgba(0,0,0,1)"}}>{app.label}</span>
          </div>
        ))}
      </div>
        ...
        <div ref={menuRef} style={{position:"fixed",bottom:52,left:0,width:340,background:"rgba(9,11,24,0.96)",backdropFilter:"blur(28px)",border:"1px solid rgba(255,255,255,0.1)",borderRadius:"0 14px 0 0",boxShadow:"6px -6px 48px rgba(0,0,0,0.65)",zIndex:9998,display:"flex",flexDirection:"column",animation:"menu-up 0.15s ease-out",overflow:"hidden"}}>
          <div style={{padding:"14px 14px 10px"}}>
            <div style={{display:"flex",alignItems:"center",gap:9,background:"rgba(255,255,255,0.07)",border:"1px solid rgba(255,255,255,0.11)",borderRadius:8,padding:"8px 12px"}}>
              ...
              {search&&<button onClick={()=>setSearch("")} style={{background:"none",border:"none",color:"rgba(255,255,255,0.3)",cursor:"pointer",fontSize:13}}>✕</button>}
            </div>
          </div>
          <div style={{padding:"0 14px 12px",flex:1,overflowY:"auto"}}>
            <div style={sHd}>All Apps</div>
  ...
  return (
    <div style={{width:390,fontFamily:FF}}>
      ...
      <div style={{display:"flex",flexDirection:"column",gap:8,marginBottom:16}}>
        ...
        <button onClick={add} style={{padding:"9px",background:acFill(AC),border:`1px solid ${acBorder(AC)}`,borderRadius:7,cursor:"pointer",fontFamily:FFB,fontWeight:600,fontSize:12,letterSpacing:0.5,color:AC}}>+ Add Note</button>
      </div>
...
function TasksApp({ data, updateData, showToast, AC }) {
  ...
  return (
    <div style={{width:360,fontFamily:FF}}>
      ...
      {pending.map(t=><TRow key={t.id} t={t} toggle={toggle} del={del} AC={AC}/>)}
      {done.length>0&&<><div style={{...sHd,marginTop:14}}>Done ({done.length})</div>{done.map(t=><TRow key={t.id} t={t} toggle={toggle} del={del} AC={AC}/>)}</>}
    </div>
  );
}
function TRow({t,toggle,del,AC}) {
  return (
    <div style={{display:"flex",alignItems:"center",gap:9,padding:"8px 11px",marginBottom:4,background:"rgba(255,255,255,0.03)",border:"1px solid rgba(255,255,255,0.07)",borderRadius:7,opacity:t.done?0.4:1,transition:"opacity 0.2s"}}>
      <div onClick={()=>toggle(t.id)} style={{width:17,height:17,borderRadius:5,border:`1.5px solid ${t.done?AC:"rgba(255,255,255,0.22)"}`,background:t.done?AC:"transparent",cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0,transition:"all 0.14s"}}>
      ...
      <button className="del-btn" onClick={()=>del(t.id)} style={{background:"none",border:"none",cursor:"pointer",color:"rgba(255,80,80,0.28)",fontSize:12,padding:0,transition:"color 0.12s"}}>✕</button>
    </div>
  );
}
...
function FilesApp({ data, updateData, showToast }) {
  ...
  if (path==="home") return (
    <div style={{width:420,fontFamily:FF}}>
      <div style={sHd}>File Explorer</div>
  ...
  if (path==="notes") return (
    <div style={{width:460,fontFamily:FF}}>
      <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:14}}>
        <button onClick={()=>{setPath("home");setPreview(null);}} style={{padding:"4px 11px",background:"rgba(255,255,255,0.07)",border:"1px solid rgba(255,255,255,0.11)",borderRadius:6,cursor:"pointer",fontFamily:FFB,fontWeight:600,fontSize:10,letterSpacing:1,color:"rgba(255,255,255,0.55)"}}>← Back</button>
        <div style={sHd}>📄 Notes</div>
      </div>
      ...
      {notes.map(n=>(
        <div key={n.id} className="file-row" onClick={()=>setPreview(n)}
          ...
          <button className="del-btn" onClick={e=>{e.stopPropagation();updateData(p=>({...p,notes:p.notes.filter(x=>x.id!==n.id)}));if(preview?.id===n.id)setPreview(null);showToast("Deleted");}}
  ...
  return (
    <div style={{width:420,fontFamily:FF}}>
      <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:14}}>
        <button onClick={()=>setPath("home")} style={{padding:"4px 11px",background:"rgba(255,255,255,0.07)",border:"1px solid rgba(255,255,255,0.11)",borderRadius:6,cursor:"pointer",fontFamily:FFB,fontWeight:600,fontSize:10,letterSpacing:1,color:"rgba(255,255,255,0.55)"}}>← Back</button>
        <div style={sHd}>📋 Tasks</div>
      </div>
...
// ─── PAINT ───────────────────────────────────────────────────────────────────
const PALETTE=["#fff","#000","#ff4444","#ff8800","#ffdd00","#44dd44","#00ccff","#4466ff","#cc44ff","#ff44aa","#8b4513","#888"];
function PaintApp({ showToast, AC }) {
  ...
  const down=(e)=>{ e.stopPropagation(); setDrawing(true); const pos=gp(e); lastPos.current=pos; const ctx=canvasRef.current.getContext("2d"); ctx.beginPath(); ctx.arc(pos.x,pos.y,size/2,0,Math.PI*2); ctx.fillStyle=tool==="eraser"?"#fff":color; ctx.fill(); };
  const move=(e)=>{ if(!drawing||!lastPos.current) return; e.stopPropagation(); const pos=gp(e); const ctx=canvasRef.current.getContext("2d"); ctx.beginPath(); ctx.moveTo(lastPos.current.x,lastPos.current.y); ctx.lineTo(pos.x,pos.y); ctx.strokeStyle=tool==="eraser"?"#fff":color; ctx.lineWidth=size; ctx.lineCap="round"; ctx.lineJoin="round"; ctx.stroke(); lastPos.current=pos; };
  const up=(e)=>{ e.stopPropagation(); setDrawing(false); lastPos.current=null; };
  const tbtn=(id,lbl)=>(
    <button onClick={()=>setTool(id)} style={{padding:"6px 12px",background:tool===id?acFill(AC):"rgba(255,255,255,0.06)",border:`1px solid ${tool===id?acBorder(AC):"rgba(255,255,255,0.11)"}`,borderRadius:6,cursor:"pointer",fontFamily:FFB,fontWeight:600,fontSize:11,color:tool===id?AC:"rgba(255,255,255,0.6)"}}>
      {lbl}
    </button>
  );
  return (
    <div style={{width:580,fontFamily:FF}}>
      <div style={{display:"flex",alignItems:"center",gap:7,marginBottom:10,flexWrap:"wrap"}}>
        {tbtn("pen","✏️ Pen")} {tbtn("eraser","⬜ Eraser")}
        <div style={{display:"flex",alignItems:"center",gap:7,marginLeft:4}}>
          ...
          <input type="range" min={2} max={40} value={size} onChange={e=>setSize(+e.target.value)} style={{width:76,accentColor:AC}}/>
          <span style={{fontSize:10,color:"rgba(255,255,255,0.5)",width:16}}>{size}</span>
        </div>
        <div style={{flex:1}}/>
        <button onClick={()=>{ const c=canvasRef.current; const ctx=c.getContext("2d"); ctx.fillStyle="#fff"; ctx.fillRect(0,0,c.width,c.height); }} style={{padding:"6px 12px",background:"rgba(255,255,255,0.06)",border:"1px solid rgba(255,255,255,0.11)",borderRadius:6,cursor:"pointer",fontFamily:FFB,fontWeight:600,fontSize:11,color:"rgba(255,255,255,0.55)"}}>Clear</button>
        <button onClick={()=>{ const a=document.createElement("a"); a.download="nova-paint.png"; a.href=canvasRef.current.toDataURL(); a.click(); showToast("Saved ✓"); }} style={{padding:"6px 12px",background:acFill(AC),border:`1px solid ${acBorder(AC)}`,borderRadius:6,cursor:"pointer",fontFamily:FFB,fontWeight:600,fontSize:11,color:AC}}>⬇ Save</button>
      </div>
      <canvas ref={canvasRef} width={560} height={340} style={{borderRadius:7,cursor:tool==="eraser"?"cell":"crosshair",display:"block",border:"1px solid rgba(255,255,255,0.1)"}} onMouseDown={down} onMouseMove={move} onMouseUp={up} onMouseLeave={up}/>
...
// ─── BROWSER — Nova Search ────────────────────────────────────────────────────
// Bing, Google, YouTube etc. all block iframes (X-Frame-Options: SAMEORIGIN).
// There is no client-side bypass for this. Nova Search uses DuckDuckGo's free
// Instant Answers API + Wikipedia's API to deliver real search results natively.
// Known iframe-friendly sites (bookmarks below) load in the browser pane directly.
...
function BrowserApp({ AC }) {
  ...
  const novaSearch=async(q)=>{
    ...
    try {
      ...
      setResults({ q, ddg:ddgRes.status==="fulfilled"?ddgRes.value:null, wiki:wikiRes.status==="fulfilled"?wikiRes.value:null });
  ...
  const back=()=>{ if(hIdx>0){ const i=hIdx-1; setHIdx(i); setFrameUrl(hist[i]); setBar(hist[i]); setView("browse"); } };
  const fwd=()=>{ if(hIdx<hist.length-1){ const i=hIdx+1; setHIdx(i); setFrameUrl(hist[i]); setBar(hist[i]); setView("browse"); } };

  const NavBar=()=>(
    <div style={{display:"flex",gap:6,marginBottom:9,alignItems:"center"}}>
      {["←","→"].map((ch,i)=>{ const en=i===0?hIdx>0:hIdx<hist.length-1; return (
        <button key={ch} onClick={i===0?back:fwd} disabled={!en} style={{width:28,height:28,borderRadius:6,background:"rgba(255,255,255,0.06)",border:"1px solid rgba(255,255,255,0.1)",cursor:en?"pointer":"default",color:"rgba(255,255,255,0.5)",fontSize:13,opacity:en?1:0.3}}>{ch}</button>
      );})}
      <input value={bar} onChange={e=>setBar(e.target.value)} onKeyDown={e=>e.key==="Enter"&&go()} placeholder="Search or enter URL…"
        style={{...inp,flex:1,fontFamily:FFM,fontSize:12}}/>
      <button onClick={()=>go()} style={{padding:"7px 14px",background:acFill(AC),border:`1px solid ${acBorder(AC)}`,borderRadius:6,cursor:"pointer",fontFamily:FFB,fontWeight:600,fontSize:12,color:AC}}>Go</button>
    </div>
  );
  ...
  if (view==="home") return (
    <div style={{width:640,fontFamily:FF}}>
      <NavBar/>
      <Bmarks/>
      <div style={{display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",height:340,gap:14,border:"1px solid rgba(255,255,255,0.07)",borderRadius:9,background:"rgba(255,255,255,0.02)"}}>
        <div style={{fontSize:44}}>🌐</div>
        <div style={{fontFamily:FFB,fontWeight:700,fontSize:18,color:"rgba(255,255,255,0.6)"}}>Nova Browser</div>
        <div style={{fontSize:12,color:"rgba(255,255,255,0.25)",textAlign:"center",lineHeight:1.7,maxWidth:380}}>
          Type a search query for <b style={{color:"rgba(255,255,255,0.45)"}}>Nova Search</b> (powered by DuckDuckGo + Wikipedia),<br/>
          or enter a URL to browse. Use bookmarks above for sites that support embedding.<br/>
          Sites like Bing/Google block iframes — use <b style={{color:"rgba(255,255,255,0.45)"}}>Bing ↗</b> to open in a new tab.
        </div>
      </div>
    </div>
  );
  ...
  if (view==="browse") return (
    <div style={{width:680,fontFamily:FF}}>
      ...
      <iframe src={frameUrl} title="nova-browser" sandbox="allow-scripts allow-same-origin allow-forms allow-popups"
        style={{width:"100%",height:420,border:"1px solid rgba(255,255,255,0.08)",borderRadius:8,background:"#fff",display:"block"}}/>
    </div>
  );

  // Nova Search results
  const ddg=results?.ddg; const wiki=results?.wiki;
  const ddgTopics=(ddg?.RelatedTopics||[]).filter(t=>t.FirstURL&&t.Text).slice(0,8);
  const wikiHits=(wiki?.query?.search||[]);
  return (
    <div style={{width:640,fontFamily:FF}}>
      <NavBar/><Bmarks/>
      {loading?(
        <div style={{display:"flex",alignItems:"center",justifyContent:"center",height:340,gap:12,flexDirection:"column"}}>
          ...
          <div style={{fontSize:12,color:"rgba(255,255,255,0.35)"}}>Searching Nova…</div>
        </div>
      ):(
        <div style={{maxHeight:440,overflowY:"auto"}}>
          <div style={{...sHd,marginBottom:10}}>Results for "{results?.q}"</div>

          {/* DDG instant answer */}
          {ddg?.AbstractText&&(
            <div style={{padding:"13px 14px",marginBottom:10,background:"rgba(79,158,255,0.08)",border:`1px solid rgba(79,158,255,0.22)`,borderRadius:9}}>
          ...
          {/* Wikipedia results */}
          {wikiHits.length>0&&<>
            <div style={sHd}>Wikipedia</div>
            {wikiHits.map(h=>(
              <div key={h.pageid} className="srcard" onClick={()=>browse(`https://en.wikipedia.org/wiki/${encodeURIComponent(h.title)}`)}
                ...
                <div style={{fontSize:11,color:"rgba(255,255,255,0.45)",lineHeight:1.55}} dangerouslySetInnerHTML={{__html:h.snippet.replace(/<[^>]*>/g,"")+"…"}}/>
          ...
          {/* DDG related topics */}
          {ddgTopics.length>0&&<>
            <div style={{...sHd,marginTop:10}}>Related</div>
            {ddgTopics.map((t,i)=>(
              <div key={i} className="srcard" onClick={()=>browse(t.FirstURL)}
          ...
          {!ddg?.AbstractText && wikiHits.length===0 && ddgTopics.length===0 && (
            <div style={{textAlign:"center",padding:"40px 0",color:"rgba(255,255,255,0.2)",fontSize:13,fontStyle:"italic"}}>No results found. Try a different search or enter a URL to browse.</div>
          )}
        </div>
      )}
    </div>
...
function TerminalApp({ user, AC }) {
  const [lines,setLines]=useState([{t:"out",v:"NOVA Terminal v3.1.0"},{t:"out",v:`Session: ${user} — ${new Date().toLocaleString()}`},{t:"out",v:'Type "help" for commands.'},{t:"gap"}]);
  ...
  const CMDS={
    ...
    neofetch:()=>[" ███╗   ██╗ ██████╗ ██╗   ██╗ █████╗ "," ████╗  ██║██╔═══██╗██║   ██║██╔══██╗"," ██╔██╗ ██║██║   ██║██║   ██║███████║"," ██║╚██╗██║██║   ██║╚██╗ ██╔╝██╔══██║"," ██║ ╚████║╚██████╔╝ ╚████╔╝ ██║  ██║",`OS: Nova v3.1  User: ${user}  Apps: ${APPS.length}`],
...
function SettingsApp({ user, data, updateSettings, showToast, AC, onCustomWallpaper }) {
  ...
  const Toggle=({label,value,onChange})=>(
    <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",padding:"10px 14px",background:"rgba(255,255,255,0.04)",border:"1px solid rgba(255,255,255,0.08)",borderRadius:8,marginBottom:6}}>
      ...
      <div onClick={()=>onChange(!value)} style={{width:40,height:22,borderRadius:11,background:value?AC:"rgba(255,255,255,0.12)",cursor:"pointer",position:"relative",transition:"background 0.2s"}}>
  ...
  return (
    <div style={{width:400,fontFamily:FF}}>
      ...
      {/* Accent color */}
      <div style={sHd}>Accent Color</div>
      <div style={{display:"flex",gap:7,marginBottom:6,flexWrap:"wrap"}}>
        {ACCENT_PRESETS.map(c=>(
          <div key={c} className="ac-preset" onClick={()=>{updateSettings({accent:c});showToast("Accent updated ✓");}}
      ...
      <div style={{fontSize:11,color:"rgba(255,255,255,0.22)",marginBottom:18,fontFamily:FFM}}>Current: {AC}</div>

      {/* Wallpaper */}
      <div style={sHd}>Wallpaper</div>
      <div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:8,marginBottom:10}}>
        {Object.entries(WALLPAPERS).filter(([k])=>k!=="custom").map(([k,w])=>(
          <div key={k} className="wp-swatch" onClick={()=>{updateSettings({wallpaper:k});showToast(`Wallpaper: ${w.name} ✓`);}}
      ...
      <button onClick={()=>fileRef.current.click()}
        style={{width:"100%",padding:"10px",background:wpId==="custom"?acFill(AC):"rgba(255,255,255,0.06)",border:wpId==="custom"?`1px solid ${acBorder(AC)}`:"1px solid rgba(255,255,255,0.12)",borderRadius:8,cursor:"pointer",fontFamily:FFB,fontWeight:600,fontSize:12,letterSpacing:0.5,color:wpId==="custom"?AC:"rgba(255,255,255,0.6)",marginBottom:20}}>
        {wpId==="custom"?"✓ Custom Wallpaper Active — Click to Change":"📁 Upload Custom Wallpaper (PNG/JPG)"}
      </button>

      {/* Window blur */}
      <div style={sHd}>Window Blur</div>
      <div style={{display:"flex",alignItems:"center",gap:10,marginBottom:18}}>
        ...
        <span style={{fontSize:11,fontFamily:FFM,color:"rgba(255,255,255,0.4)",width:28}}>{settings.winBlur||18}px</span>
      </div>

      {/* Toggles */}
      <div style={sHd}>Display</div>
      <Toggle label="24-Hour Clock" value={!!settings.clock24h} onChange={v=>updateSettings({clock24h:v})}/>
      <Toggle label="Large Text" value={settings.fontSize==="large"} onChange={v=>updateSettings({fontSize:v?"large":"normal"})}/>

      {/* Account */}
      <div style={{...sHd,marginTop:20}}>Account</div>
      <div style={{padding:"11px 14px",background:"rgba(255,255,255,0.04)",border:"1px solid rgba(255,255,255,0.08)",borderRadius:8,marginBottom:8}}>
      ...
      <button onClick={()=>{ if(window.confirm("Clear all notes and tasks? This cannot be undone.")) { updateSettings({}); showToast("Cleared data"); } }}
        style={{width:"100%",padding:"9px",background:"rgba(200,40,40,0.1)",border:"1px solid rgba(200,40,40,0.28)",borderRadius:8,cursor:"pointer",fontFamily:FFB,fontWeight:600,fontSize:12,color:"rgba(255,140,140,0.8)"}}>
        ⚠ Clear All My Data
      </button>
    </div>
  );
  ...
  return (
    <div style={{width:340,fontFamily:FF}}>
      ...
      <button onClick={()=>{updateData({bio});showToast("Bio saved ✓");}}
        style={{width:"100%",padding:"9px",background:acFill(AC),border:`1px solid ${acBorder(AC)}`,borderRadius:7,cursor:"pointer",fontFamily:FFB,fontWeight:600,fontSize:12,letterSpacing:0.5,color:AC}}>
        Save Bio
      </button>
    </div>
  );
