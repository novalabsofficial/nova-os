...
const STORE_CATALOG = [
  {id:"roblox",  name:"Roblox",       icon:"🟥",cat:"Games", url:"https://www.roblox.com",                 newTab:true, badge:"↗ New Tab",desc:"World's leading gaming platform"},
  {id:"xbox",    name:"Xbox Cloud",   icon:"🎮",cat:"Games", url:"https://www.xbox.com/en-US/play",        newTab:true, badge:"↗ New Tab",desc:"Stream Xbox Game Pass titles in your browser"},
  ...
  {id:"ps",      name:"PlayStation",  icon:"🔵",cat:"Games", url:"https://www.playstation.com/en-us/ps-now/",newTab:true,badge:"↗ New Tab",desc:"PlayStation cloud gaming"},
  ...
  {id:"poki",    name:"Poki",         icon:"🎪",cat:"Games", url:"https://poki.com",                       newTab:false,badge:"✓ In-App",desc:"Free online browser games"},
  ...
  {id:"github",  name:"GitHub",       icon:"🐙",cat:"Tools", url:"https://github.com",                     newTab:true, badge:"↗ New Tab",desc:"Code hosting and collaboration"},
  {id:"figma",   name:"Figma",        icon:"🎨",cat:"Tools", url:"https://www.figma.com",                  newTab:true, badge:"↗ New Tab",desc:"Collaborative UI design tool"},
  {id:"notion",  name:"Notion",       icon:"📓",cat:"Tools", url:"https://www.notion.so",                  newTab:true, badge:"↗ New Tab",desc:"All-in-one notes and docs workspace"},
  {id:"codepen", name:"CodePen",      icon:"✏️", cat:"Tools", url:"https://codepen.io",                     newTab:false,badge:"✓ In-App",desc:"Front-end coding environment"},
  ...
  {id:"twitter", name:"X / Twitter",  icon:"🐦",cat:"Social",url:"https://x.com",                          newTab:true, badge:"↗ New Tab",desc:"Real-time news and conversation"},
  {id:"hn",      name:"Hacker News",  icon:"🟠",cat:"News",  url:"https://news.ycombinator.com",           newTab:false,badge:"✓ In-App",desc:"Tech news, startups, programming"},
  {id:"wiki",    name:"Wikipedia",    icon:"📚",cat:"News",  url:"https://en.m.wikipedia.org",             newTab:false,badge:"✓ In-App",desc:"Free encyclopedia"},
  {id:"arxiv",   name:"arXiv",        icon:"🔬",cat:"News",  url:"https://arxiv.org",                      newTab:false,badge:"✓ In-App",desc:"Open-access research papers"},
];
    ...
    <div style={{width:"100%",fontFamily:FF}}>
      ...
      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:9}}>
        {[{id:"notes",icon:"📄",label:"Notes",count:notes.length},{id:"tasks",icon:"📋",label:"Tasks",count:tasks.length}].map(f=>(
          <div key={f.id} className="fr" onClick={()=>setPath(f.id)} style={{display:"flex",alignItems:"center",gap:11,padding:"13px 14px",background:"rgba(255,255,255,0.04)",border:"1px solid rgba(255,255,255,0.08)",borderRadius:8,cursor:"pointer",transition:"background 0.12s"}}>
...
function SnakeApp({AC}){
  ...
  useEffect(()=>{
    ...
    function onKey(e){const d=DM[e.key];if(!d)return;e.preventDefault();const s=st.current;if(d.x===-s.dir.x&&d.y===-s.dir.y)return;s.nextDir=d;}
    window.addEventListener("keydown",onKey);return()=>{window.removeEventListener("keydown",onKey);clearInterval(intv.current);};
  },[phase]);
  useEffect(()=>{draw();},[]);
  return(
    <div style={{width:"100%",fontFamily:FF,display:"flex",flexDirection:"column",alignItems:"center",gap:10}}>
      <div style={{display:"flex",gap:24,width:"100%",maxWidth:W}}>
        <div style={{fontFamily:FFB,fontWeight:600,fontSize:14,color:AC}}>🐍 SNAKE</div><div style={{flex:1}}/>
  ...
  return(
    <div style={{width:"100%",fontFamily:FF,display:"flex",flexDirection:"column",gap:12}}>
      <div style={{display:"flex",alignItems:"center",gap:10}}>
        <div style={{fontFamily:FFB,fontWeight:700,fontSize:22,color:AC}}>2048</div><div style={{flex:1}}/>
        {[["SCORE",score],["BEST",best]].map(([l,v])=>(<div key={l} style={{padding:"5px 12px",background:"rgba(255,255,255,0.08)",borderRadius:6,textAlign:"center"}}><div style={{fontFamily:FFM,fontSize:9,color:"rgba(255,255,255,0.4)",letterSpacing:1}}>{l}</div><div style={{fontFamily:FFB,fontWeight:700,fontSize:15,color:"#fff"}}>{v}</div></div>))}
      ...
      <div style={{position:"relative"}}>
        <div style={{display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:"1.5%",background:"rgba(255,255,255,0.08)",padding:"1.5%",borderRadius:10}}>
          {grid.flat().map((v,i)=>(<div key={i} style={{aspectRatio:"1",borderRadius:"8%",background:TC[v]||"#3c3a32",display:"flex",alignItems:"center",justifyContent:"center",fontFamily:FFB,fontWeight:700,fontSize:"clamp(14px,4vw,28px)",color:TT[v]||"#f9f6f2",transition:"background 0.1s"}}>{v>0?v:""}</div>))}
        </div>
...
function StoreApp({data,updateData,showToast,AC}){
  ...
  return(
    <div style={{width:"100%",fontFamily:FF}}>
        ...
        {filtered.map(app=>{const isIn=installed.includes(app.id);return(
          <div key={app.id} className="sc" style={{padding:"14px",background:"rgba(255,255,255,0.04)",border:"1px solid rgba(255,255,255,0.08)",borderRadius:10,transition:"background 0.12s"}}>
            ...
            <div style={{display:"flex",gap:7}}>
              <button onClick={()=>toggleInstall(app.id)} style={{flex:1,padding:"7px",background:isIn?"rgba(255,80,80,0.1)":"rgba(255,255,255,0.06)",border:"1px solid "+(isIn?"rgba(255,80,80,0.3)":"rgba(255,255,255,0.12)"),borderRadius:6,cursor:"pointer",fontFamily:FFB,fontWeight:600,fontSize:11,color:isIn?"rgba(255,130,130,0.9)":"rgba(255,255,255,0.6)"}}>{isIn?"– Remove":"+ Desktop"}</button>
              <button onClick={()=>window.open(app.url,"_blank")} style={{flex:1,padding:"7px",background:fill(AC),border:"1px solid "+bdr(AC),borderRadius:6,cursor:"pointer",fontFamily:FFB,fontWeight:600,fontSize:11,color:AC}}>Launch ↗</button>
            </div>
          </div>
        );})}
      ...
      {installed.length>0&&<div style={{marginTop:16,padding:"10px 14px",background:"rgba(255,255,255,0.03)",border:"1px solid rgba(255,255,255,0.07)",borderRadius:8,fontFamily:FF,fontSize:12,color:"rgba(255,255,255,0.4)"}}>{installed.length} app{installed.length!==1?"s":""} on desktop · Double-click icons to launch</div>}
    </div>
  );
}
  ...
  return(
    <div style={{width:"100%",fontFamily:FF}}>
      ...
      <div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:8,marginBottom:18}}>
        {[["📝",data?.notes?.length||0,"Notes"],["✅",(data?.tasks?.filter(t=>t.done).length||0)+"/"+(data?.tasks?.length||0),"Tasks"],["🏪",installed,"Installed"]].map(([ic,v,k])=>(<div key={k} style={{padding:"11px 10px",background:"rgba(255,255,255,0.03)",border:"1px solid rgba(255,255,255,0.07)",borderRadius:8,textAlign:"center"}}><div style={{fontSize:10,marginBottom:3}}>{ic}</div><div style={{fontFamily:FFB,fontWeight:700,fontSize:20,color:AC}}>{v}</div><div style={{fontSize:10,color:"rgba(255,255,255,0.32)",marginTop:2}}>{k}</div></div>))}
      </div>
